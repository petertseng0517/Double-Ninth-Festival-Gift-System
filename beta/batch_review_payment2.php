...
if (isset($_POST['submit'])) {
    ...
    for ($i = 0; $i < count($_POST['pid']); $i++) {
        ...
        $sql = "UPDATE request_money SET name=?, gift_money=?, reviewdate=? , Payment_method=? WHERE pid=?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssss", $name, $gift_money, $reviewdate, $Payment_method, $id);
        mysqli_stmt_execute($stmt);
    }   
