...
<h1 style="font-weight: bold;font-size: 36px;">百歲人瑞發送重陽禮金清單</h1>
説明：因為百歲人瑞的禮金，可能會依照長官指示有所變動，故此表不顯示禮金金額。
...
// 檢查使用者是否已登入，以及操作權限
    ...
    if (isset($_POST['submit'])) {
        ...
        // 設定 SQL 查詢語句
        $sql_centenarians = "SELECT * from ca
							WHERE ca.township = '$selectedTownship'
							and ca.birthday <= '0121231'
							ORDER BY ca.township ASC";

        // 執行 SQL 查詢
        $result_centenarians = mysqli_query($conn, $sql_centenarians);
         
		echo mysqli_num_rows($result_centenarians) ; //搜尋結果筆數
		 
		 
        // 檢查查詢結果是否有資料
        if (mysqli_num_rows($result_centenarians) > 0) {
            echo "<h2>查詢結果：</h2>";

            // 顯示查詢結果
			 
            echo "<table>";
            echo "<tr><th>PID</th><th>姓名</th><th>生日</th><th>縣市</th><th>鄉鎮</th><th>村里</th><th>鄰</th><th>地址</th></tr>";

            while ($row = mysqli_fetch_assoc($result_centenarians)) {
                echo "<tr>";
                echo "<td>" . $row['pid'] . "</td>";
